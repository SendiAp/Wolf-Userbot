...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    drg = await sticcers[c_lick].click("me", hide_via=True)
    if drg:
        await borg.send_file(int(chat_id), drg, reply_to=reply_to_id)
        await drg.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    drg = await sticcers[0].click("me", hide_via=True)
    if wolf:
        await bot.send_file(int(chat_id), wolf, reply_to=reply_to_id)
        await wolf.delete()
