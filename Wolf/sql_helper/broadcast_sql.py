...
class DrgBroadcast(BASE):
    __tablename__ = "wolfbroadcast"
    ...
    def __repr__(self):
        return "<Dragons Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, DrgBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


DrgBroadcast.__table__.create(checkfirst=True)

DRGBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with DRGBROADCAST_INSERTION_LOCK:
        broadcast_group = DrgBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with DRGBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(DrgBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with DRGBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(DrgBroadcast.keywoard)
            .filter(DrgBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(DrgBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(DrgBroadcast).count()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(DrgBroadcast).all()
        for x in all_groups:
