...
def get_collection(keywoard):
    try:
        return SESSION.query(Drg_GlobalCollection_Json).get(keywoard)
    finally:
        SESSION.close()
...
def add_collection(keywoard, json, njson):
    ...
    if not to_check:
        keyword_items = Drg_GlobalCollection_Json(keywoard, json, njson)
...
def del_collection(keywoard):
    ...
    if not to_check:
        return False
    keyword_items = SESSION.query(Drg_GlobalCollection_Json).get(keywoard)
...
def get_collections():
    try:
        return SESSION.query(Drg_GlobalCollection_Json).all()
    finally:
        SESSION.close()
