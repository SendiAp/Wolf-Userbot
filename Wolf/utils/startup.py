...
async def setup_bot():
    """
    To set up bot for Wolf
    """
    try:
        await wolf.connect()
        config = await wolf(functions.help.GetConfigRequest())
        for option in config.dc_options:
            if option.ip_address == wolf.session.server_address:
                if drgub.session.dc_id != option.id:
                    LOGS.warning(
                        f"Fixed DC ID in session from {wolf.session.dc_id}"
                        f" to {option.id}"
                    )
                wolf.session.set_dc(option.id, option.ip_address, option.port)
                wolf.session.save()
                break
        bot_details = await wolf.tgbot.get_me()
        Config.TG_BOT_USERNAME = f"@{bot_details.username}"
        # await wolf.start(bot_token=Config.TG_BOT_USERNAME)
        wolf.me = await drgub.get_me()
        wolf.uid = wolf.tgbot.uid = utils.get_peer_id(wolf.me)
        if Config.OWNER_ID == 0:
            Config.OWNER_ID = utils.get_peer_id(wolf.me)
        ...
        with open(name) as f:
            ...
            try:
                if shortname.replace(".py", "") not in Config.NO_LOAD:
                    ...
                    os.remove(Path(f"Wolf/{folder}/{shortname}.py"))
            except Exception as e:
                os.remove(Path(f"Wolf/{folder}/{shortname}.py"))
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        try:
            entity = await wolf.get_entity(BOTLOG_CHATID)
