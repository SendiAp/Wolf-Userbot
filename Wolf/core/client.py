...
class WolfUserBotClient(TelegramClient):
    def wolf_cmd(
        self: TelegramClient,
        ...
        **kwargs,
        ...
        if pattern is not None:
            if (
                ...
                reg1 = "\\" + Config.CMD_HANDLER
                reg2 = "\\" + Config.SUDO_CMD_HANDLER
            ...
            async def wrapper(check):
                ...
                try:
                    ...
                    if not disable_errors:
                        ...
                        text = "**Wolf-Userbot Error report**\n\n"
                        link = "[here](https://t.me/Wolf_UserbotSupport)"
            ...
            async def wrapper(check):
                try:
                    ...
                    if not disable_errors:
                        ...
                        text += f"**Error report : ** [{new['error']}]({astelink})"
                        await check.client.send_message(
                            Config.PRIVATE_GROUP_BOT_API_ID, text, link_preview=False
                        )

            from .session import walf

            if edited is True:
                walf.tgbot.add_event_handler(func, events.MessageEdited(**kwargs))
            else:
                walf.tgbot.add_event_handler(func, events.NewMessage(**kwargs))
...
WolfUserBotClient.fast_download_file = download_file
WolfUserBotClient.fast_upload_file = upload_file
WolfUserBotClient.reload = restart_script
WolfUserBotClient.get_msg_link = get_message_link
WolfUserBotClient.check_testcases = checking
try:
    send_message_check = TelegramClient.send_message
except AttributeError:
    WolfUserBotClient.send_message = send_message
    WolfUserBotClient.send_file = send_file
    WolfUserBotClient.edit_message = edit_message
