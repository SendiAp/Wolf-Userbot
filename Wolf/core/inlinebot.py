...
WOLFLOGO = "https://telegra.ph/file/6b64dcce161d8f3493f41.jpg"
...
async def inline_handler(event):  # sourcery no-metrics
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**Wolf"):
            buttons = [
                (
                    Button.inline("Stats", data="stats"),
                    Button.url("Repository", "https://github.com/SendiAp/Wolf-Userbot"),
                )
            ]
            ALIVE_PIC = gvarstatus("ALIVE_PIC")
            IALIVE_PIC = gvarstatus("IALIVE_PIC")
            if IALIVE_PIC:
                WOLF = [x for x in IALIVE_PIC.split()]
                PIC = list(DRG)
                I_IMG = random.choice(PIC)
            if not IALIVE_PIC and ALIVE_PIC:
                WOLF = [x for x in ALIVE_PIC.split()]
                PIC = list(DRG)
                I_IMG = random.choice(PIC)
            elif not IALIVE_PIC:
                I_IMG = None
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        wolf = f"[{u.first_name}](tg://user?id={u.id})"
                except ValueError:
                    # ValueError: Could not find the input entity
                    wolf = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                ...
                text=f"🔒 Pesan sembunyi untuk {wolf} , Hanya dia yang bisa membukanya",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            _result = main_menu()
            result = builder.article(
                title="(C) Wolf-Userbot Help",
                description="Menu Help Dari Wolf-Userbot",
                text=_result[0],
                buttons=_result[1],
                link_preview=False,
            )
            ...
            if WOLF_IMG and WOLF_IMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    WOLF_IMG,
                    # title="Alive Dragons",
                    text=query,
        ...
        markup = event.client.build_reply_markup(buttons)
        photo = types.InputWebDocument(
            url=WOLFLOGO, size=0, mime_type="image/jpeg", attributes=[]
        )
        text, msg_entities = await event.client._parse_message_text(
            "Anda dapat deploy __Wolf-Userbot__ Anda Sendiri\n\n**Klik Button Url Dibawah Ini**", "md"
        )
        result = types.InputBotInlineResult(
            id=str(uuid4()),
            type="photo",
            ...
            url="https://github.com/SendiAp/Wolf-Userbot",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
...
@wolf.tgbot.on(CallbackQuery(data=re.compile(b"close")))
@check_owner
...
@wolf.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@wolf.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@wolf.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
